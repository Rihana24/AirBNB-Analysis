...
def datafr():
    df= pd.read_csv("C:/Users/pc/Desktop/AB/Airbnb.csv")
    return df
...
if select == "Home":
    ...
    image1= Image.open("C:/Users/pc/Desktop/AB/image.jpg")
    st.image(image1)

    st.header("About Airbnb")
    st.write("")
    st.write('''***Airbnb was born in 2007 when two Hosts welcomed three guests to their San Francisco home,
              and has since grown to over 4 million Hosts who have welcomed over 1.5 billion guest arrivals
              in almost every country across the globe. Airbnb is an online marketplace that connects people
              who want to rent out their property with people who are looking for accommodations, typically 
              for short stays. Airbnb offers hosts a relatively easy way to earn some income from their 
              property.Guests often find that Airbnb rentals are cheaper and homier than hotels.***''')
    st.write("")
...
if select == "Data Exploration":
    ...
    with tab1:
        ...
        with col1:
            ...
            fig_bar= px.bar(df_bar, x='property_type', y= "price", title= "PROPERTY_TYPES WITH PRICE",hover_data=["number_of_reviews","review_scores"],color_discrete_sequence=px.colors.sequential.Redor_r, width=600, height=500)
        ...
        with col2:
            ...
            fig_pi= px.pie(df_pie, values="price", names= "host_response_time",
                            ...
                            title="PRICE DIFFERENCE WITH HOST RESPONSE TIME",
        ...
        with col1:
            ...
            title='MINIMUM NIGHT AND MAXIMUM NIGHT',hover_data="price",
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv("C:/Users/pc/Desktop/AB/Airbnb.csv")
            return df_a
        ...
        roomtype_a= st.selectbox("Select the Room Type_available", df2_a["room_type"].unique())
    ...
    with tab3:
        ...
        country_l= st.selectbox("Select the Country_location",df_l["country"].unique())
    ...
    with tab5:
        ...
        country_t= st.selectbox("Select the Country_top_charts",df["country"].unique())

        df1_t= df[df["country"] == country_t]

        property_ty_t= st.selectbox("Select the Property_type_top_charts",df1_t["property_type"].unique())
...
if select == "About":
    ...
    st.write('''***Gathered data from Airbnb's public API or other available sources.
    ...
    st.write('''***Clean and preprocessed the data to handle missing values, outliers, and ensure data quality.
        Convert data types, handle duplicates, and standardize formats.***''')
    
    st.subheader(":orange[3. Exploratory Data Analysis (EDA):]")

    st.write('''***Conducted exploratory data analysis to understand the distribution and patterns in the data.
        Explore relationships between variables and identify potential insights.***''')
    
    st.subheader(":orange[4. Visualization:]")

    st.write('''***Created visualizations to represent key metrics and trends.
    ...
    st.write('''***Utilized geospatial analysis to understand the geographical distribution of listings.
